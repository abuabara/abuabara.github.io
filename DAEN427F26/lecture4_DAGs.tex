...
\subtitle{Lecture 4: \textbf{Directed Acyclic Graphs}}
\author{}
\date{
\color{aggiemaroon}{
...
\textcolor{red}{A DAG is causal only when the arrows are assigned a causal
interpretation and the required assumptions are defensible.}

\medskip

\textcolor{blue}{Correlation alone does not tell us which way an arrow should point.}

\end{frame}

% ------------------------------------------------------------
\begin{frame}{Three patterns}
...
\textit{After we know \(Z\), learning \(X\) gives us no additional information
about \(Y\).}
...
\begin{itemize}
\item For variables \(X_1,\ldots,X_n\), a Bayesian network represents
...
\item Instead of specifying one enormous joint probability distribution, we
specify several smaller conditional models.

\item For binary variables, an unrestricted joint distribution over \(n\)
...
\item A sparse Bayesian network can require dramatically fewer.
\end{itemize}
\end{frame}
...
\begin{tikzpicture}[node distance=1.3cm]
  ...
  \node[dagObserved, below right=.7cm and 0.3cm of rain3]
    (grass3) {Wet\\grass};
...
\begin{tikzpicture}[node distance=1.3cm]
  ...
  \node[dagDecision, below=.9cm of age]
    (maintain) {Maintain?};

  \node[dagUtility, right=1.3cm of failure]
    (cost) {Total\\value};
...
The goal is to choose the action with the best expected consequences.
...
\begin{tikzpicture}[node distance=.7cm]
  ...
  \node[dagDecision, right=1.cm of inspection]
    (maintenance2) {Maintain?};

  \node[dagUtility, above right=.9cm and 1.2cm of maintenance2]
    (value2) {Value};
...
\begin{block}{A useful modeling habit}
