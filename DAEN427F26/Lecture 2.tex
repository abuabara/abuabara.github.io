...
\begin{frame}{}
\centering
%\includegraphics[height=8.2cm]{Fake News Detection on Social Media.jpg}
...
Before reading, we can use prior knowledge from historical data.
...
\begin{itemize}
    ...
    P(B \mid A) \neq P(B)
    \]
    \item If knowing \(A\) does not change the probability of \(B\),\\
...
\begin{itemize}
    ...
    \item \textcolor{aggiemaroon!60}{\emph{Example}: rolling a die and flipping a coin are independent, because the die result \textbf{does not} affect the coin flip.}
\end{itemize}
\end{block}
\end{frame}
...
\begin{itemize}
    ...
    \item Suppose we already observed an article with exclamation points.
...
Likelihood $(\cdot \mid A)$ & 0.267 & 0.022 & \hl{0.289} \\
\hline
\end{tabular}
\end{table}
\end{block}
