...
\begin{ideia}
\textbf{Main idea:} first, we define what would make a process problematic.
...
\begin{atencao}
Comparing $0.8337$ directly with $0.25$ is not the decision rule. To choose an
action, we must compare $q$ with the action threshold $\tau$.
\end{atencao}
...
\begin{ideia}
\textbf{Remember:} $\theta_0$ defines the problem, $q$ measures our
