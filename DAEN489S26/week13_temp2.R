...
map_life <- ggplot(data1) +
  geom_sf(aes(fill = PctDiplomaE), color = NA, size = 0) +
  scale_fill_viridis_c(
    option = "magma",
    name = "Percent"
  ) +
  theme_void() +
  labs(title = "HS Diploma or More") +
  theme(
...
ggplot() +
  ...
  geom_sf(data = subset(data1, is.na(MedianIncomeE) | is.na(PctDiplomaE)),
...
mcar_test(st_drop_geometry(select(data1, MedianIncomeE, PctDiplomaE)))
...
vis_miss(st_drop_geometry(select(data1, MedianIncomeE, PctDiplomaE)))

data_test <- data1 %>%
  mutate(MedianIncomeMissing = ifelse(is.na(MedianIncomeE), 1, 0))

# simple logistic regression (or t-test): does PctDiploma predict whether MedianIncome is missing?
summary(glm(MedianIncomeMissing ~ PctDiplomaE, data = data_test, family = "binomial"))

################

# remove missing values
data2 <- subset(data1, !is.na(MedianIncomeE) & !is.na(PctDiplomaE))

nrow(data1) - nrow(data2) # droping: 3510 tracts

cor.test(log10(data2$MedianIncomeE), data2$PctDiplomaE)

################

library(geostan)

sp_diag(data2$PctDiplomaE, data2, name = "pct_diploma")
sp_diag(data2$MedianIncomeE, data2, name = "income USD")

A <- shape2mat(data2, "B", method = "rook")

A <- shape2mat(data2, "B", method = "rook", snap = 4000)
...
summary(glm(log(MedianIncomeE) ~ PctDiplomaE, weights = PopTotalE, data = data2))

fit_lm <- stan_glm(log(MedianIncomeE) ~ PctDiplomaE,
