...
ggplot(data, aes(x = pct_with_diploma, y = "")) +
  geom_boxplot(color = "navy",
               ...
               outliers = TRUE) +
  labs(
    # title    = "Distribution of pct_with_diploma",
    # subtitle = "Texas Counties",
    caption  = "Source: ACS 5-Year Data (2008-2022)",
    y = "",
    x = "Percentage of adults with a Bachelor's degree or higher") + theme_minimal()
...
sigma(multilevel_model1)

# resids <- resid(multilevel_model1)
# data$residuals <- resid(multilevel_model1)
# resid(multilevel_model1, type = "pearson")
# marginal_resids <- data$med_income_k - predict(multilevel_model1, re.form = NA)
...
dev.off()
plot(zoom_area[, "med_income_k"], main = "County ID / med_income_k", reset = FALSE)
...
# sp_diag(data$med_income_k, data, rates = FALSE)
